...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState, warning: bool) -> &'static str {
    if warning && state == TrayIconState::Idle {
        return match theme {
            ...
            AppTheme::Colored => "resources/bizum_voice.png",
        };
    }
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/bizum_voice.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Bizum Voice v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Bizum Voice v{}", env!("CARGO_PKG_VERSION"))
    }
}
